...
import pl.uam.wmi.niezbednikstudenta.services.NotificationService;
...
public class NotificationController {
    ...
    public void register(@RequestParam String token) {
        notificationService.subscribeToTopic("admin", token);
    }
    ...
    @GetMapping
    public ResponseEntity getAllUserNotifications(HttpServletRequest request, @ApiIgnore Pageable pageable) {
        ...
        try {
            ...
            return new ResponseEntity(notificationService.getNotificationsForUser(user, pageable), HttpStatus.OK);
    ...
    public ResponseEntity deleteUserNotification(HttpServletRequest request, @PathVariable Long notificationId) {
        ...
        try {
            User user = userService.getAndCheckIfUserHasPermission(token, secretToken, false);
            notificationService.deleteUserNotification(user, notificationId);
    ...
    @GetMapping("/is-read")
    public ResponseEntity checkIfUserReadNotifications(HttpServletRequest request) {
        ...
        try {
            User user = userService.getAndCheckIfUserHasPermission(token, secretToken, false);
            return new ResponseEntity(user.isUserReadNotifications(), HttpStatus.OK);
        } catch (InterruptedException | NoSuchAlgorithmException | InvalidKeyException | IOException | SecurityException e) {
            throw new ResponseStatusException(HttpStatus.FORBIDDEN, e.getMessage(), e);
        }
    }
